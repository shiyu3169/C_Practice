...
double great_circle_distance(double lat1, double lon1, double lat2, double lon2) {
  if (lat1 < -90 || lat1 > 90 || lat2 < -90 || lat2 > 90 ||
    ...
    return -1.0; }
  else { double lat01 = lat1 * 3.14159/180;
         double lat02 = lat2 * 3.14159/180;
         double lon01 = lon1 * 3.14159/180;
         double lon02 = lon2 * 3.14159/180;
         double dlon = lon02  - lon01;
         double dlat = lat02  - lat01;
         double a = (sin(dlat/2)) * (sin(dlat/2)) + cos(lat01) * cos(lat02) * (sin(dlon/2)) * (sin(dlon/2));
